        ...
        try:
            ...
            with redirect_stdout(stdout_capture), redirect_stderr(stderr_capture):
                ...
                if args.device.lower() == 'cuda' and torch.cuda.is_available():
                    # 使用显式的设备映射而不是'auto'，确保视觉模块正确分配到GPU
                    device_map = {
                        ...
                        "model.vision_tower": 0,  # 确保视觉塔分配到GPU 0
                        "model.image_newline": 0  # 确保image_newline参数分配到GPU 0
                    }
                    ...
                    logger.info("使用显式设备映射确保视觉模块正确分配到GPU")
                    if not args.silent:
                        print(f"使用GPU加速，CUDA可用: {torch.cuda.is_available()}")
                        print("使用显式设备映射确保视觉模块正确分配到GPU")
                else:
                    device_map = 'cpu'
                    target_device = "cpu"
                    logger.info("使用CPU模式")
                ...
                try:
                    ...
                    if hasattr(model, 'device'):
                        print(f"主模型设备: {model.device}")
                        logger.info(f"主模型设备: {model.device}")
                    
                    # 检查模型参数设备
                    model_params = list(model.parameters())
                    if model_params:
                        first_param_device = model_params[0].device
                        print(f"模型参数设备: {first_param_device}")
                        logger.info(f"模型参数设备: {first_param_device}")
                    ...
                    if hasattr(model, 'get_vision_tower') and model.get_vision_tower() is not None:
                        ...
                        if vision_params:
                            vision_device = vision_params[0].device
                            print(f"视觉塔设备: {vision_device}")
                            logger.info(f"视觉塔设备: {vision_device}")
                    
                    # 检查多模态投影器设备
                    if hasattr(model, 'get_model') and hasattr(model.get_model(), 'mm_projector'):
                        mm_projector = model.get_model().mm_projector
                        if hasattr(mm_projector, 'parameters'):
                            mm_params = list(mm_projector.parameters())
                            if mm_params:
                                mm_device = mm_params[0].device
                                print(f"多模态投影器设备: {mm_device}")
                                logger.info(f"多模态投影器设备: {mm_device}")
                    
                    # 检查image_newline参数设备
                    if hasattr(model, 'get_model') and hasattr(model.get_model(), 'image_newline'):
                        image_newline = model.get_model().image_newline
                        print(f"image_newline参数设备: {image_newline.device}")
                        logger.info(f"image_newline参数设备: {image_newline.device}")
                    ...
                    if torch.cuda.is_available():
                        ...
                        print(f"GPU显存使用: {torch.cuda.memory_allocated()/1024**3:.2f}GB / {torch.cuda.memory_reserved()/1024**3:.2f}GB")
                    else:
                        print(f"CUDA可用: 否")
                    ...
                    if target_device != "cpu":
                        ...
                        if hasattr(model, 'get_vision_tower') and model.get_vision_tower() is not None:
                            vision_tower = model.get_vision_tower()
                            vision_tower = vision_tower.to(target_device)
                            logger.info(f"视觉塔已移动到设备: {target_device}")
                            if not args.silent:
                                print(f"视觉塔已移动到设备: {target_device}")
                        
                        # 移动多模态投影器
                        if hasattr(model, 'get_model') and hasattr(model.get_model(), 'mm_projector'):
                            model.get_model().mm_projector = model.get_model().mm_projector.to(target_device)
                            logger.info(f"多模态投影器已移动到设备: {target_device}")
                        
                        # 移动image_newline参数（如果存在）
                        if hasattr(model, 'get_model') and hasattr(model.get_model(), 'image_newline'):
                            model.get_model().image_newline = model.get_model().image_newline.to(target_device)
                            logger.info(f"image_newline参数已移动到设备: {target_device}")
                            if not args.silent:
                                print(f"image_newline参数已移动到设备: {target_device}")
                        
                        # 确保整个模型在正确的设备上
                        model = model.to(target_device)
                        logger.info(f"整个模型已移动到设备: {target_device}")
                    else:
